            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="(row, i) in data.slice(2, )" :key="i">
                <td
                  ...
                  :key="idx" class="sm:px-6 px-1 py-4 sm:text-sm text-xs font-bold text-gray-800 whitespace-nowrap">{{item}}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  fetch() {
    this.$axios
      .$get(
        `https://sheets.googleapis.com/v4/spreadsheets/1SWikQcKkDgSTXOUW8JD2b4qcOSTlkEGYh6XE9fS7aL8/values/Sheet1!A7:F18?key=AIzaSyBSWyF6kNY2qB4KklKubjos4u_hNIOFx_I`
      )
      .then((resp) => {
        // const res = resp.values
